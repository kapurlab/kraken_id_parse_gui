...
class Apicomplexa():
    ...
    def order_segments_by_coverage(self, virus_dict):
        ...
        segment_order = {
            'CHROMOSOME 1': 1, 'CHROMOSOME 2': 2, 'CHROMOSOME 3': 3, 'CHROMOSOME 4': 4, 'MITOCHONDRION': 5,
            'APICOPLAST': 6
        }
        ...
        for seq_id, seq_info in virus_dict.items():
            ...
            # Check for chromosome/organellar notation
            for seg_name, seg_num in segment_order.items():
                if seg_name in header:
                    segment_num = seg_num
                    break
            
            # Check for direct segment number notation if no chromosome found
            if segment_num is None:
                ...
                if segment_matches:
                    try:
                        ...
                        if 1 <= potential_segment <= 6:  # Valid segment range
    ...
    def run(self, alignment_stats=None):
        ...
        print(f"Downloading {len(dict_ordered)} Apicomplexa sequences...")
        downloaded_count = 0
        failed_downloads = []
        
        for i, (seq_id, seq_inf) in enumerate(dict_ordered.items(), 1):
            print(f"Downloading sequence {i}/{len(dict_ordered)}: {seq_id}")
        ...
        if downloaded_count > 0:
            print("Proceeding with concatenation of successfully downloaded sequences...")
