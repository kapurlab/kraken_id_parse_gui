...
export default function App() {
  ...
  // JSON parsing → runs were mislabelled "Failed"). It also truncates buffered
  // responses at ~43.5 KB, so logs/results never refreshed. /api/jobs/{id}/logtext
        ...
        .then((data) => {
          ...
          if (typeof data.log === "string") {
            ...
            for (let i = lines.length - 1; i >= 0; i--) {
              const d = lines[i];
              if (/Step \d+:/i.test(d) || /Starting bioinformatics/i.test(d) ||
                  /Generating analysis reports/i.test(d) || /Pipeline completed/i.test(d) ||
                  /Downloading.*reference/i.test(d)) {
