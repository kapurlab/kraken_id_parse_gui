# Kraken ID Parse — OOD sandbox app (Slurm, no admin)

Per-user deployment of Kraken ID Parse on any Open OnDemand HPC. The card appears
...
bin/bdtools install --sandbox kraken_id_parse_gui        # builds env+frontend, links this card
$EDITOR ~/.../ood/apps/kraken_id_parse_gui_sandbox/form.yml   # set cluster: "CHANGE_ME" -> your cluster
```

`bdtools install --sandbox kraken_id_parse_gui` writes `~/.config/kraken_id_parse_gui/sandbox.env`
(`BDTOOLS_APP_DIR` + `BDTOOLS_APP_ENV`) and symlinks this card into
`~/ondemand/dev/kraken_id_parse_gui`. The launcher (`template/script.sh.erb`) sources
